...
- Sempre que uma skill iniciar listener/banco **manualmente** (`lsnrctl start`, `startup`) fora do systemd, ao final da atividade **parar manualmente e subir via `systemctl restart`** para o systemd assumir o controle do processo.
...
- **Usar sempre `systemctl restart`, nunca `systemctl start`, neste passo** (testado/confirmado na Atividade 5, 2026-08-13): os serviços usam `RemainAfterExit=yes`, então depois de qualquer bounce manual anterior na mesma atividade o systemd pode continuar achando o serviço `active` mesmo com o processo real morto — `systemctl start` nesse caso é **no-op silencioso** (`is-active` mente "active" sem nenhum `pmon` de pé). `restart` força o `ExecStart` a rodar de verdade independente do estado anterior.
- **Confirmar sempre com `ps aux | grep pmon`/`tnslsnr`, nunca só `systemctl is-active`** — `is-active` de um serviço `RemainAfterExit=yes` não garante que o processo real existe.
- Se após o `restart` a query der `ORA-01034` mesmo com `is-active` dizendo `active`: possível SGA órfã sem `pmon` (`dbstart` do 19c pode tentar iniciar a mesma instância duas vezes internamente e deixar esse estado inconsistente). Recuperar com `shutdown abort` + `startup` manual antes de repetir o `systemctl restart`.
- **Este duplo-start do `dbstart` é recorrente, não um evento isolado** (confirmado 3x: Atividade 5, duas vezes na Atividade 6, e de novo na Atividade 7 — em homes 19c e 23ai, com e sem parâmetros depreciados no spfile). Não é algo que se "conserta" de vez removendo uma causa específica — tratar como **passo padrão esperado** em todo handoff pós-mudança de Oracle Home: depois de qualquer `systemctl restart` de banco recém-movido, sempre conferir `ps aux | grep pmon`; se faltar, aplicar direto `sqlplus -prelim / as sysdba` → `SHUTDOWN ABORT` → `systemctl restart` de novo (a segunda tentativa após o abort sempre funcionou limpo nas 3 ocorrências).
